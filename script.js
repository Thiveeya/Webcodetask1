 ...
 var arr_probability = [];             //Variable to get an probablity data
  for (i = 0; i < data1.country.length; i++) {
    arr_country.push(data1.country[i].country_id);           // getting an country data into an array 
      ...
      // console.log(Math.max(arr_probability)); 
    //  If needed here we can get the majority result
  }
  ...
  var top2ele=arr_country;
  var top2ele1=arr_probability;
  for (j = 0; j < top2ele.length; j++) {
    top2ele=arr_country.slice(0,2);            //getting top 2 elements 
    top2ele1=arr_probability.slice(0,2);       //getting top 2 probability

    var tr2 = document.createElement("tr")       // creating an tr2
    var td2 = document.createElement("td")        // creating an td2
    td2.innerHTML = `${top2ele[j]}`
    var td3 = document.createElement("td")       //  creating an td3
    td3.innerHTML = `${top2ele1[j]}`
  ...
  table.append(tbody)
 }         // Finally appending it into Tbody 
